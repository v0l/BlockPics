    ...
    class Program
    {
        ...
        static async Task SendPics(string host, string token)
        {
            ...
            while (IsRunning)
            {
                ...
                igen.Arguments = $"-y -f rawvideo -pix_fmt rgb24 -s {sres}x{sres} -i - -vframes 1 {block_hash}.png";
                ...
                try
                {
                    ...
                    await authClient.PostNewStatus($"#Bitcoin tip updated! {block_hash}", null, new List<int>() { media.Id });
